...
if __name__ == "__main__":
    ...
    model_path = '/home/hzchen/jyh/llm_models/Qwen/Qwen3-8B/'
    tokenizer = hf_tokenizer(model_path)
    from verl.trainer.main_ppo import create_rl_sampler
    from torchdata.stateful_dataloader import StatefulDataLoader
    train_dataset = RLHFDatasetWithTarget(parquet_files=data_path,
        tokenizer=tokenizer,
        prompt_key='prompt',
        max_prompt_length=1024,
        filter_prompts=True, return_raw_chat=False,
        truncation='error',
        max_target_length=8192,
        filter_targets=False,
        sample_target_ratio=1.0)
    ...
    for batch in train_dataloader:
        print(batch)
        print(type(batch))
        print(type(batch['reward_model']))
        new_batch: DataProto = DataProto.from_single_dict(batch)
        print(new_batch)
        input_str = tokenizer.decode(new_batch.batch['input_ids'][0], skip_special_tokens=False)
        print(input_str)
        print('pad token: ', tokenizer.decode(tokenizer.pad_token_id), ' id: ', tokenizer.pad_token_id)
        print('eos token: ', tokenizer.decode(tokenizer.eos_token_id), ' id: ', tokenizer.eos_token_id)
        print(new_batch.batch.keys())
        print(new_batch.non_tensor_batch.keys())
        print(new_batch.non_tensor_batch)
        break